...
Frame* Animation::getFrame(int frameNumber)
{
	...
	list<Frame>::iterator i = frames.begin(); //iterator is something that helps you naviagate through a list, point iterator to first frame in the frames list
	int counter = 0;
	...
	Frame* frame = &(*i);	 // make frame point to the frame inside the list our iterator is pointing at
							// *i = gets frame at i's position in the list (dereferencing)
